...
# GET DATA LAMBDA ROLE/POLICIES
resource "aws_iam_role" "get_data" {
  assume_role_policy = data.aws_iam_policy_document.get_data_assume_policy.json
}

data "aws_iam_policy_document" "get_data_assume_policy" {
  statement {
    actions = ["sts:AssumeRole"]
    principals {
      type        = "Service"
...
data "aws_iam_policy_document" "get_data_policy_document" {
  statement {
    actions = [
      "logs:CreateLogGroup",
      "logs:CreateLogStream",
...
resource "aws_iam_policy" "get_data_policy" {
  name   = "get_data_policy"
  policy = data.aws_iam_policy_document.get_data_policy_document.json
}

resource "aws_iam_role_policy_attachment" "get_data_policy_attachment" {
  role       = aws_iam_role.get_data.name
  policy_arn = aws_iam_policy.get_data_policy.arn
}
